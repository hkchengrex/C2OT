...
from c2ot.model.unet_utils import (avg_pool_nd, conv_nd, linear, normalization, timestep_embedding,
                                   zero_module)
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        # weight = th.einsum("bct,bcs->bts", q * scale,
        ...
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        a = F.scaled_dot_product_attention(q, k, v).transpose(1, 2)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.continuous_code = continuous_code
        ...
        if self.conditional:
            if self.num_classes is not None:
                if continuous_code:
                    self.label_emb = nn.Linear(code_dim, time_embed_dim)
                else:
